...
import { logError, logInfo, logSuccess, logWarn, printList, printStep } from "../ui/output.js";
...
const ensureUniqueKeyPath = async (sshDir: string, keyPath: string): Promise<string> => {
  ...
  while (existsSync(candidate) || existsSync(`${candidate}.pub`)) {
    const overwrite = await promptYesNo(`Key already exists at ${candidate}. Overwrite?`, false);
    if (overwrite) {
      return candidate;
    }
